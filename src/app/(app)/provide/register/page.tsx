    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 pb-32 space-y-5 max-w-lg mx-auto">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">{categories.map(cat => (<button key={cat.id} type="button" onClick={() => setForm({...form, categoryId: cat.id})} className={`p-3 rounded-2xl border-2 text-center transition-all ${form.categoryId === cat.id ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50"}`}><div className="text-2xl mb-1">{cat.icon}</div><div className={`text-xs font-semibold line-clamp-1 ${form.categoryId === cat.id ? "text-blue-700" : "text-gray-900"}`}>{cat.name}</div></button>))}</div>
        </div>
